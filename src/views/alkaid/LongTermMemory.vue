    ...
    >
      <div>
        ...
        <div style="display: flex; gap: 8px">
          <v-btn color="primary" variant="tonal" @click="onNodeSelect">
            <v-icon start> mdi-magnify </v-icon>
            {{ tm("filters.filterButton") }}
          </v-btn>
          <v-btn color="secondary" variant="tonal" @click="resetFilter">
            <v-icon start> mdi-filter-remove </v-icon>
            {{ tm("filters.resetButton") }}
          </v-btn>
          <v-btn color="primary" variant="tonal" @click="refreshGraph">
                ...
                <v-expansion-panel-title>
                  <div>
                    <span
                      ...
                      >{{ result.text.substring(0, 30) }}...</span
                    >
                    <span class="ms-2 text-caption text-grey"
                      >({{ tm("search.similarity") }}:
                  ...
                  <div>
                    ...
                    <div class="d-flex">
                      <span class="text-caption text-grey"
                        >{{ tm("factDialog.docId") }}: {{ result.doc_id }}</span
                      >
                    </div>
                  </div>
                </v-expansion-panel-text>
      ...
      <div v-if="graphStats" class="mt-4">
        ...
        <v-card variant="outlined" class="mt-2 pa-3">
          <div class="d-flex justify-space-between">
            <span class="text-subtitle-2"
              >{{ tm("graphStats.nodeCount") }}:</span
            >
            <span>{{ graphStats.nodeCount }}</span>
          </div>
          <div class="d-flex justify-space-between">
            <span class="text-subtitle-2"
              >{{ tm("graphStats.edgeCount") }}:</span
            >
      ...
      <v-dialog v-model="showFactDialog" max-width="550" scrollable>
        <v-card class="fact-detail-card">
          <v-card-title
            class="d-flex align-center bg-primary text-white px-4 py-3"
          >
          ...
          <v-card-actions v-if="selectedEdgeFactData" class="pa-4">
            <v-btn
              block
              color="primary"
              variant="tonal"
...
export default {
  ...
  methods: {
        ...
        .then((response) => {
          if (response.data.status === "ok") {
            ...
            if (this.searchResults.length === 0) {
              ...
              this.$toast.success(
                this.tm("messages.searchSuccess", {
    ...
    addMemoryData() {
      ...
      axios
        .post("/api/plug/alkaid/ltm/graph/add", payload)
        .then((response) => {
          // 成功添加后刷新图表
          this.refreshGraph();
    ...
    ltmGetUserIds() {
      axios
        .get("/api/plug/alkaid/ltm/user_ids")
        .then((response) => {
          // 确保返回的数据是数组类型
          const data = response.data.data;
