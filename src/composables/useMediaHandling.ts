...
export function useMediaHandling() {
  ...
  const stagedImagesUrl = computed(() => stagedFiles.value.filter((f) => f.type === "image").map((f) => f.url));

  const stagedAudioUrl = computed(() => stagedFiles.value.find((f) => f.type === "record")?.url || "");
